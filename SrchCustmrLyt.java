...
class SrchCustmrLyt implements ActionListener,KeyListener
{
    JFrame f;
    JPanel p;
    ...
    public SrchCustmrLyt()
    {
        ...
        update.setBounds(490,245,100,30);
        ...
        p.add(update);p.add(clr);p.add(blnc);
    ...
    public void actionPerformed(ActionEvent evt)
    {
        if(evt.getSource()==update)
        {
            ...
            int result=customerqueries.updateCustomer(names.getText().toString().trim(),nos.getText().toString().trim(),addrss.getText().toString().trim(),totals.toString().trim());
            if(result==1)
            {
                JOptionPane.showMessageDialog(f,"Customer Updated","Customer",JOptionPane.PLAIN_MESSAGE);
                f.setVisible(false);
                f.dispose();
            }
            else
            {
                JOptionPane.showMessageDialog(f,"Customer Not Updated","Customer",JOptionPane.PLAIN_MESSAGE);
            }
        }
    }
    ...
    public void keyReleased(KeyEvent evt)
    {
        try
        {
            ...
            totals=totl.getText().toString().trim();
        }
        catch(Exception e)
        {
        }
